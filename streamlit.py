...
def search_callback():
    st.session_state.df = hf.get_tweet_df(
...
with st.sidebar:
    ...
    st.markdown(
        """
            ...
            7 days based on the entered search term. Since the app can only predict 
            positive or negative sentiment, it is more suitable towards analyzing 
            the sentiment of brand, product, service, company, or person. 
...
if "df" in st.session_state:
    ...
    def make_dashboard(tweet_df, bar_color, wc_color):
        ...
        with col1:
            ...
            st.dataframe(
                tweet_df[["Sentiment", "Like Count", "Tweet"]].style.applymap(
